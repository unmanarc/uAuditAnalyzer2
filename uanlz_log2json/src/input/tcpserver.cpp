...
bool logServerThr(void * obj, StreamSocket * baseClientSocket, const char * remotePair, bool secure)
{
    ...
    Globals::getAppLog()->log1(__func__,sRemotePair,Logs::LOG_LEVEL_INFO,"Receiving %srsyslog+auditd incomming connection...", secure? "secure ":"");
...
void TCPServer::startThreaded()
{
    ...
    {
        Globals::getAppLog()->log0(__func__,Logs::LOG_LEVEL_ERR,"Error creating TCP LOG listener @%s:%lu",listenAddr.c_str(),listenPort);
        return;
    }

    Globals::getAppLog()->log0(__func__,Logs::LOG_LEVEL_WARN,"LOG Listener (with decoder:%s) listener running on TCP @%s:%lu...", decoder.c_str(), listenAddr.c_str(),tcpServer->getPort());
